...
class hpcm : hpwin_sketch<int> {
public:
	hpcm(size_t memory, int hash_num, int win_num) : win_num(win_num) {
		csk = new cmsketch*[win_num];
		
		for (int i = 0; i < win_num; ++i)
			csk[i] = new cmsketch(memory / win_num, hash_num);
	}
