...
class hop_basic_cu : hpwin_sketch<int> {
public:
	hop_basic_cu(size_t memory, int size, int hf_num, int per = 1) : size(size), period(per) {
		size_t mem = memory / size;
		sk = new cusketch*[size];
		lst = 0;
		counter = 0;
		for (int i = 0; i < size; ++i)
			sk[i] = new cusketch(mem, hf_num);
	}
	~hop_basic_cu() {
		for (int i = 0; i < size; ++i)
			delete sk[i];
